...
const MaestriaCuencas: React.FC = () => {
  const content = PAGE_CONTENT['/posgrado/maestrias/cuencas-hidrograficas'];
  const [data, setData] = useState<MaestriaData | null>(null);
  ...
  useEffect(() => {
    const fetchMaestria = async () => {
      try {
        const response = await fetch('/datajson/maestria-cuencas.json');
        if (!response.ok) {
      ...
      <div className="prose prose-lg dark:prose-invert max-w-none">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-20 text-gray-500 dark:text-gray-400">
            ...
            <p className="text-sm font-bold uppercase tracking-wider">Cargando información de la Maestría...</p>
          </div>
        ) : error || !data ? (
          ...
          <div className="space-y-12 animate-fadeIn text-left">
            ...
            <div className="bg-gradient-to-br from-emerald-500/5 via-sky-500/5 to-transparent border border-gray-150 dark:border-slate-800 rounded-[40px] p-6 md:p-10 shadow-sm space-y-6">
              <div className="flex items-start gap-4">
                <div className="p-4 bg-sky-50 dark:bg-sky-950/40 text-sky-600 dark:text-sky-400 rounded-3xl shadow-sm shrink-0">
                  <Droplets size={32} />
                </div>
                <div>
                  <span className="text-[10px] font-black uppercase tracking-widest text-sky-600 dark:text-sky-400 block mb-1">
                    Programa Académico de Maestría de Posgrado
                  </span>
            ...
            <div className="space-y-6 pt-4">
              ...
              <div className="bg-[#f0f9ff]/50 dark:bg-sky-950/10 border border-sky-100/60 dark:border-sky-900/20 p-5 rounded-2xl flex items-center justify-between text-sky-950 dark:text-sky-300">
                <span className="text-[11px] font-black uppercase tracking-widest flex items-center gap-2">
                  ...
                  Estructura Curricular de Cuencas Hidrográficas
                </span>
...
export default MaestriaCuencas;
