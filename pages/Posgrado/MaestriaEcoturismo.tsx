...
const MaestriaEcoturismo: React.FC = () => {
  const content = PAGE_CONTENT['/posgrado/maestrias/ecoturismo'];
  ...
  useEffect(() => {
    const fetchMaestria = async () => {
      try {
        const response = await fetch('/datajson/maestria-ecoturismo.json');
        if (!response.ok) {
            ...
            <div className="space-y-6 pt-4">
              ...
              <div className="bg-[#f0f9ff]/50 dark:bg-sky-950/10 border border-sky-100/60 dark:border-sky-900/20 p-5 rounded-2xl flex items-center justify-between text-sky-950 dark:text-sky-300">
                <span className="text-[11px] font-black uppercase tracking-widest flex items-center gap-2">
                  ...
                  Estructura Curricular de Ecoturismo
                </span>
...
export default MaestriaEcoturismo;
