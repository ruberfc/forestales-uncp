...
import { Loader2, AlertCircle, Award, Target, Shield, Trees, FileText } from 'lucide-react';

interface PerfilEgresoData {
  titulo: string;
  descripcion: string;
  competencias: string[];
...
const Egreso: React.FC = () => {
  const content = PAGE_CONTENT['/pregrado/perfil-estudiante/egreso'];
  const [data, setData] = useState<PerfilEgresoData | null>(null);
    ...
    >
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 text-gray-500 dark:text-gray-400">
          ...
          <p className="text-sm font-bold uppercase tracking-wider">Cargando perfil de egreso...</p>
        </div>
      ) : error || !data ? (
        ...
        <div className="space-y-12 animate-fadeIn max-w-5xl mx-auto">
          ...
          <div className="bg-[#f0f9ff]/60 dark:bg-sky-950/10 border border-sky-100 dark:border-sky-900/20 rounded-[40px] p-6 md:p-12 shadow-sm space-y-8 text-left">
            ...
            <div className="space-y-4">
              {data.competencias.map((comp, idx) => (
                ...
                >
                  <div className="w-8 h-8 rounded-full bg-emerald-600 text-white flex items-center justify-center font-black text-sm shrink-0 shadow-sm">
                    {idx + 1}
                  </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 pt-4">
            <div className="bg-gradient-to-br from-emerald-500/5 to-transparent border border-emerald-500/10 rounded-2xl p-6 text-left">
              <Trees className="text-emerald-500 mb-3" size={24} />
              <h4 className="font-black text-slate-800 dark:text-white text-xs uppercase tracking-wider mb-1">Manejo Ecosistémico</h4>
              <p className="text-[11px] text-gray-500 dark:text-gray-400 leading-relaxed">
                Gestión, aprovechamiento y transformación sostenible de bienes y servicios del ecosistema forestal.
              </p>
            </div>
            <div className="bg-gradient-to-br from-sky-500/5 to-transparent border border-sky-500/10 rounded-2xl p-6 text-left">
              ...
              <h4 className="font-black text-slate-800 dark:text-white text-xs uppercase tracking-wider mb-1">Gestión Ambiental</h4>
              <p className="text-[11px] text-gray-500 dark:text-gray-400 leading-relaxed">
                Desarrollo y aplicación de sistemas integrados y políticas de protección ambiental con liderazgo social.
              </p>
            </div>
            <div className="bg-gradient-to-br from-[#84cc16]/5 to-transparent border border-[#84cc16]/10 rounded-2xl p-6 text-left">
              <Award className="text-[#84cc16] mb-3" size={24} />
              <h4 className="font-black text-slate-800 dark:text-white text-xs uppercase tracking-wider mb-1">Investigación e Impacto</h4>
              <p className="text-[11px] text-gray-500 dark:text-gray-400 leading-relaxed">
                Solución innovadora de problemas complejos con sustento científico para el beneficio de comunidades locales y globales.
              </p>
            </div>
          </div>
        </div>
